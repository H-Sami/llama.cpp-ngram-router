...
std::string server_task_result_metrics::to_metrics() {
    ...
    const auto & controller = metrics.spec_controller;
    if (controller.enabled) {
        const std::vector<metric_item> controller_counters = {
            { "spec_controller_decisions_total", "Controller: Total routing decisions", (double) controller.decisions },
            { "spec_controller_mtp_selections_total", "Controller: MTP-only selections", (double) controller.mtp_selections },
            { "spec_controller_ngram_selections_total", "Controller: Standalone n-gram selections", (double) controller.ngram_selections },
            { "spec_controller_fused_selections_total", "Controller: MTP-to-n-gram fused selections", (double) controller.fused_selections },
            { "spec_controller_abstentions_total", "Controller: Ordinary target decode selections", (double) controller.abstentions },
        ...
        const auto add_producer_counter = [&](const char * name, const char * description, const auto & value) {
            prometheus << "# HELP llamacpp:" << name << " " << description << "\n"
                       << "# TYPE llamacpp:" << name << " counter\n";
            for (const auto & producer : controller.producers) {
                prometheus << "llamacpp:" << name
                           << "{producer_id=\"" << producer.producer_id
                           << "\",configuration=\"" << escape_label(producer.configuration_name)
                           << "\",type=\"" << escape_label(producer.type) << "\"} "
                           << value(producer) << "\n";
            }
        };
        add_producer_counter("spec_controller_producer_proposals_total", "Controller: Candidate spans generated by producer", [](const auto & p) { return p.proposals; });
        add_producer_counter("spec_controller_producer_proposed_tokens_total", "Controller: Candidate span tokens generated by producer", [](const auto & p) { return p.proposed_tokens; });
        add_producer_counter("spec_controller_producer_selections_total", "Controller: Selected candidate spans by producer", [](const auto & p) { return p.selections; });
        add_producer_counter("spec_controller_producer_selected_tokens_total", "Controller: Selected candidate span tokens by producer", [](const auto & p) { return p.selected_tokens; });
